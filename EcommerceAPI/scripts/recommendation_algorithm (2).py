...
def select_candidate_set(auth_user_id):
    ...
    for friends_shelves in auth_user_friends_with_shelves:
        result = {"len": len(auth_user_shelves.intersection(set(friends_shelves["list_shelves"]))),
                  "friend_id": friends_shelves["friend_id"]}
        result_with_len.append(result)
    result_with_len.sort(key=operator.itemgetter('len'))
...
start = time.clock()
print(resem(631932, 631932))
# print close(gr_api.get_user_Xbooks(gr_api.get_auth_user_id())[0].get("list_shelf"), 46109817)
